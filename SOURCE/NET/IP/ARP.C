...
void arp_request(unsigned int daddr, struct s_net_device *dev)
{
  ...
  if (net_arp_debug)
    printk("arp.c: Sending arp request for IP: %s",in_ntoa(daddr));
...
void arp_update_cache(struct s_net_buff *buff)
{
  ...
  if (arp_entry == NULL)
  {
     alert("File: arp.c  Function: arp_update_cache\n\n%s",
           "No mem left to add arp-cache entry.\n");
    return;
  }
...
int arp_get_mac(unsigned int daddr, unsigned char *ha_addr)
{
  ...
  if (net_arp_debug)
    printk("arp.c: arp_get_mac for %s",in_ntoa(daddr));
...
void arp_rcv(struct s_net_buff *buff, struct s_net_device *dev)
{
  ...
  switch (htons(arphdr->ar_op))
  {
    ...
    default:
       {
          alert("File: arp.c  Function: arp_rcv\n\n%s%d%s",
                "ARP Opcode ",
                htons(arphdr->ar_op),
