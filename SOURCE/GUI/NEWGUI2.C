...
void draw_win_c(rect_t *rect, window_t *win)
{
  ...
  couts_c(rect, win->rect.x1+6,win->rect.y1+6,COLOR_WHITE,win->name);
}
...
int pt_inrect2(rect_t *rect, int x, int y)
{
...
void size_win(int dx, int dy, int mx, int my)
{
  ...
  {
    domask_win(x1,y2+1,x2+dx,y2+dy);
...
window_t *create_app(void *pTask, char *pname, char *wname, int x1, int y1, int length, int width)
{
  ...
  domask_win(win->rect.x1,win->rect.x1,win->rect.x2,win->rect.y2);
...
//            RemoveTask(win_app->ptask);
            break;
       case MSG_GUI_REDRAW:
            win_app->flags &= ~WF_STOP_OUTPUT;
            app_redraw(-1,n,m);
            win_app->flags &= ~WF_MSG_GUI_REDRAW;
            break;
     }
