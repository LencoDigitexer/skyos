...
static int sprint_dev_config(struct pci_dev *dev, char *buf, int size)
{
	...
	pcibios_read_config_byte (bus, devfn, PCI_MAX_LAT, &max_lat);
        ...
        if ((class_rev < 0x0f) || (class_rev == 0xff))
        {
            sprintf(buffer,"%s",pci_strdev(vendor, device));
            buffer[254] = '\0';
            register_hardware(class_rev, buffer, 0);
        }
        else if (class_rev >= 0x100)
        {
           devman_class = (class_rev >> 8);
           sprintf(buffer,"%s",pci_strdev(vendor, device));
           buffer[254] = '\0';
           register_hardware(devman_class, buffer, 0);
        }
...
#	define MSG "\nwarning: page-size limit reached!\n"
	...
	size  = 4096 - (strlen(MSG) + 1);
	len   = sprintf(buf, "PCI devices found:\n");

        printk("pci.c: PCI device found:");

	for (dev = pci_devices; dev; dev = dev->next)
        {
		nprinted = sprint_dev_config(dev, buf, size);
                printk("%s",buf);
                sleep(18);
...
static void *pci_malloc(long size/*, unsigned long *mem_startp*/)
{
        ...
        mem = (void*)valloc((size + sizeof(void*) - 1) & ~(sizeof(void*) - 1));
        memset(mem, 0 , size);
        return mem;
...
static unsigned int scan_bus(struct pci_bus *bus)
{
...
#ifdef DEBUG
                printk("Alloc memory <1>");
                sleep(18);
#endif

		dev = pci_malloc(sizeof(struct pci_dev));
		dev->bus = bus;
		dev->devfn  = devfn;
		dev->vendor = l & 0xffff;
...
#ifdef DEBUG
                        printk("Insering into tree. <4>");
                                sleep(18);
#endif
			/*
			 * Insert it into the tree of buses.
			 */
			child = pci_malloc(sizeof(struct pci_bus));
...
#ifdef DEBUG
                 printk("Insering into tree done. <5>");
                 sleep(18);
#endif
...
unsigned long pci_init(void)
{
	...
	if (!pcibios_present())
        {
		printk("pci.c: no BIOS32 detected\n");
		return 0;
	}
...
//#ifdef DEBUG
	{
                int len;
                mem_start = (unsigned int*)valloc(4096);

		len = get_pci_list((char*)mem_start);
/*		if (len)
                 {
